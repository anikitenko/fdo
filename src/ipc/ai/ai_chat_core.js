...
export async function compressSessionMessages(session, event, llm, assistantInfo, sessions, idx) {
    ...
    try {
        ...
        resp = await withTpmRetry(() => llm.chat(textBlock));
    } catch (err) {
    ...
    const totalTokens = Math.max(0, prevTotalTokens - adjustedUsed);
...
export async function handleStreamingResponse(
    ...
    let resp;
    try {
        resp = await withTpmRetry(() => llm.chat({
            ...llmOptions,
        }));
    } catch (err) {
    ...
    if (resp && typeof resp === "object" && "stream" in resp && typeof resp.complete === "function") {
        for await (const chunk of resp.stream) {
            ...
            if (type === "content" && piece && typeof piece === "string") {
                ...
                const delta = computeDelta(prevFull, full);
...
export async function handleNonStreamingResponse(
    ...
    llm.user(content);
    const resp = await withTpmRetry(() => llm.chat({
        ...llmOptions,
    }));
...
function computeDelta(prev, next) {
    console.log("[AI Chat] computeDelta:", prev, next);
    if (!prev) return next;

    // Find the suffix of `next` not present in `prev`
    let i = 0;
    while (i < prev.length && prev[i] === next[i]) i++;
    let rawDelta = next.slice(i);

    // Normalize garbage from repeated syllables or stutters
    // 1) "wordwordword" → "word"
    rawDelta = rawDelta.replace(/\b(\w+)\1{1,}\b/g, "$1");

    // 2) "word word word" → "word"
    rawDelta = rawDelta.replace(/\b(\w+)(\s+\1){1,}\b/g, "$1");

    // 3) punctuation stutter: ",," → ","
    rawDelta = rawDelta.replace(/([,.;!?])\1+/g, "$1");

    // 4) character stutter: "AAAI" → "AI"
    rawDelta = rawDelta.replace(/(.)\1{2,}/g, "$1$1");

    return rawDelta.trimStart();
}
