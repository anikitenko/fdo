...
const EXAMPLE_RELATIVE_PATH = "fixtures/operator-terraform-plugin.fixture.ts";
const EXPECTED = {
  handlers: [
...
function resolveExamplesRootForThisSpec() {
  ...
  throw new Error(`Unable to resolve SDK examples root for terraform fixture spec. Tried: ${preferred}, ${fallback}`);
}
...
async function getFixtureUiSnapshot(window) {
  return await window.evaluate(() => {
    ...
    return {
      ...
      output: String(doc?.getElementById("terraform-workflow-result")?.innerText || "").trim(),
      hasPreviewButton: !!doc?.getElementById("terraform-preview-plan"),
      hasWorkflowButton: !!doc?.getElementById("terraform-preview-apply-workflow"),
    };
  });
}
...
async function waitForFixtureUiReady(window, timeoutMs = 30000) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    const ready = snapshot.hasPreviewButton && snapshot.hasWorkflowButton && snapshot.text.includes("Fixture: Terraform Operator");
...
async function clickButtonAndWaitOutput(window, buttonId, timeoutMs = 12000) {
  return await window.evaluate(async ({ buttonId, timeoutMs }) => {
    ...
    const output = doc?.getElementById("terraform-workflow-result");
...
test.describe("SDK fixture operator-terraform-plugin: live E2E line proof", () => {
  ...
  test("terraform fixture: envelopes + host path response + UI/logs/coverage", async () => {
    test.setTimeout(220000);
    const window = await electronApp.firstWindow();
    const pluginName = "sdk-e2e-fixture-operator-terraform-dedicated";
    ...
    try {
      ...
      const previewEnvelope = await invokeHandler(window, pluginName, "terraformFixture.v2.previewPlan", {});
      ...
      const workflowEnvelope = await invokeHandler(window, pluginName, "terraformFixture.v2.previewApplyWorkflow", {});
      ...
      const clickPreview = await clickButtonAndWaitOutput(window, "terraform-preview-plan", 14000);
      ...
      const clickWorkflow = await clickButtonAndWaitOutput(window, "terraform-preview-apply-workflow", 18000);
