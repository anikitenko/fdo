...
const EXAMPLE_RELATIVE_PATH = "09-operator-plugin.ts";
const EXPECTED = {
  metadataName: "Operator Plugin Example",
  handler: "operator.buildDockerStatusRequest",
  uiMarkers: [
    "Operator Plugin Example",
    "Dry-Run Docker Status",
    "createOperatorToolActionRequest(\"docker-cli\", ...)",
  ],
  coverageMinimums: {
    lines: 80,
    statements: 80,
...
function resolveExamplesRootForThisSpec() {
  ...
  throw new Error(`Unable to resolve SDK examples root for 09 spec. Tried: ${preferred}, ${fallback}`);
}
...
async function get09UiSnapshot(window) {
  return await window.evaluate(() => {
    const iframe = document.querySelector('iframe[title="Plugin Container ID"]');
    const doc = iframe?.contentDocument;
    const output = String(doc?.getElementById("operator-result-box")?.innerText || "").trim();
    const runButton = doc?.getElementById("run-docker-status");
...
async function waitFor09UiReady(window, timeoutMs = 30000) {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeoutMs) {
    const snapshot = await get09UiSnapshot(window);
    const ready = snapshot.hasRunButton && snapshot.iframeText.includes("Operator Plugin Example");
    if (ready) {
      return snapshot;
    }
    await window.waitForTimeout(180);
  }
  return get09UiSnapshot(window);
}
...
async function clickRunAndWait(window, matcherExpr, timeoutMs = 10000) {
  return await window.evaluate(async ({ matcherExpr, timeoutMs }) => {
    ...
    const runButton = doc.getElementById("run-docker-status");
    const resultBox = doc.getElementById("operator-result-box");
...
test.describe("SDK example 09-operator-plugin: live E2E line proof", () => {
  ...
  test("09 demo: curated operator envelope + denied/success dry-run path", async () => {
    test.setTimeout(200000);
    const window = await electronApp.firstWindow();
    const pluginName = "sdk-e2e-09-operator-plugin-dedicated";
    ...
    try {
      ...
      expect(envelopeResponse?.result?.request?.action).toBe("system.process.exec");
      expect(envelopeResponse?.result?.request?.payload?.scope).toBe("docker-cli");
      expect(envelopeResponse?.result?.request?.payload?.dryRun).toBe(true);
      expect(envelopeResponse?.result?.request?.payload?.command).toBe("/usr/local/bin/docker");
      expect(envelopeResponse?.result?.request?.payload?.args).toEqual(["ps", "--format", "json"]);
      expect(String(envelopeResponse?.result?.correlationId || "")).toMatch(/^docker-cli-/);

      // Denied path first (without required operator grants).
      ...
      const baseSnapshot = await waitFor09UiReady(window, 30000);
      ...
      await waitFor09UiReady(window, 20000);

      const successResult = await clickRunAndWait(
        window,
        "text.includes('\"status\": \"ok\"') && text.includes('\"dryRun\": true') && text.includes('\"scope\": \"docker-cli\"')"
      );
