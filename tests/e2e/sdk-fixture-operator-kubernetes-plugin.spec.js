...
const EXAMPLE_RELATIVE_PATH = "fixtures/operator-kubernetes-plugin.fixture.ts";
const EXPECTED = {
  handlers: [
  ...
  uiMarkers: [
    "Fixture: Kubernetes Operator",
    "kubectlFixture.v2.*",
    "Inspect Objects",
    "Inspect + Restart Workflow",
    "UI_MESSAGE",
  ],
  coverageMinimums: {
    lines: 80,
...
function resolveExamplesRootForThisSpec() {
  ...
  throw new Error(`Unable to resolve SDK examples root for kubernetes fixture spec. Tried: ${preferred}, ${fallback}`);
}
...
async function getFixtureUiSnapshot(window) {
  return await window.evaluate(() => {
    ...
    return {
      ...
      output: String(doc?.getElementById("kubectl-workflow-result")?.innerText || "").trim(),
      hasPreviewButton: !!doc?.getElementById("kubectl-preview-objects"),
      hasWorkflowButton: !!doc?.getElementById("kubectl-inspect-restart-workflow"),
    };
  });
}
...
async function waitForFixtureUiReady(window, timeoutMs = 30000) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    const ready = snapshot.hasPreviewButton && snapshot.hasWorkflowButton && snapshot.text.includes("Fixture: Kubernetes Operator");
...
async function clickButtonAndWaitOutput(window, buttonId, timeoutMs = 12000) {
  return await window.evaluate(async ({ buttonId, timeoutMs }) => {
    ...
    const output = doc?.getElementById("kubectl-workflow-result");
...
test.describe("SDK fixture operator-kubernetes-plugin: live E2E line proof", () => {
  ...
  test("kubernetes fixture: envelopes + host path response + UI/logs/coverage", async () => {
    test.setTimeout(220000);
    const window = await electronApp.firstWindow();
    const pluginName = "sdk-e2e-fixture-operator-kubernetes-dedicated";
    ...
    try {
      ...
      const declaredCaps = Array.isArray(diagnostics?.capabilities?.declaration?.declared)
        ...
        : [];
      expect(declaredCaps.length).toBeGreaterThan(0);
      ...
      const previewEnvelope = await invokeHandler(window, pluginName, "kubectlFixture.v2.previewClusterObjects", {});
      ...
      const workflowEnvelope = await invokeHandler(window, pluginName, "kubectlFixture.v2.inspectAndRestartWorkflow", {});
      expect(workflowEnvelope.ok).toBe(true);
      const workflowEnvelopePayload = extractEnvelopePayload(workflowEnvelope);
      expect(workflowEnvelopePayload?.action).toBe("system.workflow.run");

      const workflowHost = await invokeHandler(window, pluginName, "requestPrivilegedAction", workflowEnvelopePayload || {});
      expect(typeof workflowHost?.ok).toBe("boolean");
      if (workflowHost.ok) {
        expect(String(workflowHost.correlationId || "")).not.toBe("");
      } else {
        const errText = `${workflowHost?.error || ""} ${workflowHost?.code || ""}`.trim();
        expect(errText.length).toBeGreaterThan(0);
      }

      const clickPreview = await clickButtonAndWaitOutput(window, "kubectl-preview-objects", 14000);
      expect(clickPreview.ok, JSON.stringify(clickPreview)).toBe(true);
      expect(/ok|error|code|correlationId|workflow|stdout|stderr/i.test(clickPreview.text)).toBe(true);

      const clickWorkflow = await clickButtonAndWaitOutput(window, "kubectl-inspect-restart-workflow", 18000);
      expect(clickWorkflow.ok, JSON.stringify(clickWorkflow)).toBe(true);
      expect(/ok|error|code|correlationId|workflow|steps|summary|stdout|stderr/i.test(clickWorkflow.text)).toBe(true);
